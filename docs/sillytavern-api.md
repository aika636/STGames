...
**Ширина содержимого (ST 1.18.0).** Попап заметно уже вьюпорта: на экране 360 CSS-px тело
попапа с `wider: true` даёт содержимому **276px** (замер в живой таверне под Playwright,
2026-08-08). То есть на поля уходит ~84px — почти четверть ширины телефона. Из-за этого
`92vw` в `--sudoku-size` (331px) не влезает и режется `max-width: 100%`. Вывод для вёрстки игр:
**на `vw` полагаться нельзя**, элементы должны переживать сжатие — ширины через
`minmax(0, 1fr)`, квадратность через `aspect-ratio`, размеры шрифта — в `cqw` от самого
элемента, а не от переменной с желаемым размером.

**`cqw` нельзя писать на том же элементе, который объявляет контейнер** (замер в живой
таверне, 2026-08-11). Контейнер не запрашивает сам себя: `container-type: inline-size`
и `font-size: …cqw` на одном селекторе дают размер не от этого элемента, а от ближайшего
внешнего контейнера — а если его нет, от вьюпорта. В широком окне ST кроссворд получил
буквы 106px вместо 30px: сетка 358px, но `58cqw` посчитались от вьюпорта 1280px. Правило:
`container-type` — на доске, `font-size: …cqw` — на клетке (как в сапёре и нонограмме).
На телефоне ошибка не видна вовсе — там ширина доски почти равна ширине экрана, поэтому
ловится она только замером в живой таверне на широком окне.
...
* Как доска и змейка выглядят в светлой теме. Мобильная ширина судоку проверена
  (360×740 под Playwright, ST 1.18.0); змейка на узком экране — глазами не смотрели.
